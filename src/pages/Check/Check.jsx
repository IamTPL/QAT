...
const Check = () => {
    ...
    const retrieveData = useCallback(async () => {
        ...
        try {
            ...
            const response = await ApiClass.getAll(params, 'check');
            if (
    ...
    const columns = [
                ...
                <span
                    onClick={() => {
                        if (record.status === 'success') {
                            navigate(`/check-review/${record.id}`);
                        } else {
                            if (!isMessageVisible) {
        ...
        selectedColumns.Status && {
            ...
            key: 'status',
            width: '14%',
            render: (status) => <UploadStatus status={status} />,
        },
        selectedColumns['Date Upload'] && {
            ...
            key: 'dateUpload',
            width: '14%',
            render: (date) => <span className="text-[15px]">{date}</span>,
        },
        selectedColumns.Actions && {
            ...
            render: (_, record) => (
                <ActionDropdown
                    ...
                    onDeleteItem={() => handleDelete(record.id, record.status)}
    ...
    useEffect(() => {
        const saveSelectedColumns = localStorage.getItem(
            'selectedColumns/checks'
        );
    ...
    const handleDeleteSelectedItems = async () => {
        ...
        const response = await ApiClass.deleteSelected(
            deletedItemsRequest,
            'check'
        );
    ...
    const handleDownloadSelectedItems = async () => {
        ...
        await DocumentResultCheckService.downloadXLSXFiles(selectedItems);
        setSelectedItems([]);
        showSuccess();
    };
    ...
    const handleDownloadItem = async (id) => {
        try {
            await DocumentResultCheckService.downloadXLSXFiles([id]);
            setSelectedItems([]);
            showSuccess();
        } catch (error) {
    ...
    //catch processing items and initiate iterval task
    ...
    const handleConvertSelectedItems = async () => {
        ...
        if (newAndFailedItems.length > 0) {
            ...
            try {
                const response = await DocumentService.covertCheckSelected(
                    documentIds
                );
    ...
    const handleConvertItem = async (id) => {
        try {
            const response = await DocumentService.covertCheckSelected([id]);
            if (
